...
test_token_file_exists() {
    ...
    local token_file="$HOME/.config/gdrive-curl/tokens.json"
    assert_file_exists "$token_file" "Token file exists"
}
...
test_token_file_valid_json() {
    ...
    if [[ -f "$token_file" ]]; then
        if jq empty "$token_file" 2>/dev/null; then
            pass "Token file contains valid JSON"
        else
            fail "Token file does not contain valid JSON"
        fi
    else
...
test_token_has_required_fields() {
    ...
    if [[ -f "$token_file" ]]; then
        ...
        if [[ -n "$has_access_token" ]]; then
            pass "Token file has access_token"
        else
            fail "Token file missing access_token"
        fi

        if [[ -n "$has_refresh_token" ]]; then
            pass "Token file has refresh_token"
        else
            fail "Token file missing refresh_token"
        fi
    else
